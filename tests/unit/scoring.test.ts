...
class MockScoringService {
  ...
  normalizeIntent(intent: string, commercialScore: number): number {
    ...
    const baseScore = baseScores[intent as keyof typeof baseScores] || 0.4;
...
describe('ScoringService', () => {
  ...
  describe('batchCalculateScores', () => {
    ...
    it('should handle large batches efficiently', async () => {
      ...
      const largeKeywordSet = Array.from({ length: 1000 }, (_, i) => ({
        ...mockKeywords[0],
        id: `kw-${i}`,
      ...
      const results = await scoringService.batchCalculateScores(largeKeywordSet);
  ...
  describe('detectQuickWins', () => {
    ...
    it('should consider volume percentile in quick win detection', () => {
      const keywordSet = mockKeywords.map((kw, i) => ({
        ...kw,
        difficulty: 15,
        volume: (i + 1) * 1000 // Varying volumes
      }));

      const results = keywordSet.map(kw => scoringService.calculateBlendedScore(kw));
      const quickWins = scoringService.detectQuickWins(results);

      // Higher volume keywords should be more likely to be quick wins
      const highVolumeQuickWins = quickWins.filter(qw => qw.volume >= 2000 && qw.quickWin).length;
      const lowVolumeQuickWins = quickWins.filter(qw => qw.volume < 2000 && qw.quickWin).length;

      expect(highVolumeQuickWins).toBeGreaterThanOrEqual(lowVolumeQuickWins);
    });
  });
